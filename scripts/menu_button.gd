...
func select(highlight_num):
	if option_num == highlight_num and not setting_slide and visible:
		if setting_rebind and not rebinding:
			...
			line_edit.editable = not line_edit.editable
...
func _input(event):
	if rebinding:
		...
		if not first_input:
			...
			var can_rebind = true
			if (event is InputEventJoypadMotion and ((event.axis_value > - 0.8 and event.axis_value < 0.8) or not rebind_axis)) or ((event is InputEventKey or event is InputEventJoypadButton) and not event.pressed):
				can_rebind = false
