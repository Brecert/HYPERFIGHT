...
export  var character = global.CHAR.goto
...
func set_highlight(player_num):
	if player_num > 0:
		...
		z_index = 0
		if curr_anim != "deselect" and not label_player1.visible and not label_player2.visible:
...
func select_up_override():
	var new_portrait = null
	match character:
		global.CHAR.goto:
			new_portrait = global.CHAR.random
		global.CHAR.yoyo:
			new_portrait = global.CHAR.random
		global.CHAR.random:
			new_portrait = global.CHAR.sword
	return find_portrait(new_portrait)

func select_down_override():
	var new_portrait = null
	match character:
		global.CHAR.time:
			if global.unlock_char_darkgoto:
				new_portrait = global.CHAR.darkgoto
			else :
				new_portrait = global.CHAR.random
		global.CHAR.random:
			new_portrait = global.CHAR.goto
	return find_portrait(new_portrait)
