...
func _ready():
	...
	if player_num == 2:
		...
		char_spr.set_position(Vector2(char_spr.get_position().x * - 1, char_spr.get_position().y))
	...
	orig_y = get_position().y
	if y_scroll_offset != 0:
		set_position(Vector2(get_position().x, get_position().y + y_scroll_offset))
	
	anim_player.playback_speed = 0
	anim_player.play("goto")
	
	char_spr.get_material().set_shader_param("threshold", 0.001)
	char_spr.get_material().set_shader_param("white_amount", 0)
...
func set_char(char_name):
	if not selected:
		...
		if curr_char != char_name:
			char_spr.set_position(Vector2(get_position().x + select_x, char_spr.get_position().y))
		...
		label_name.text = global.get_char_real_name(char_name) + "READY!"
...
func process(delta):
	...
	char_spr.set_position(Vector2(char_spr.get_position().x + (target_x - char_spr.get_position().x) / 4 * (global.fps * delta), char_spr.get_position().y))
	if scrolling:
		set_position(Vector2(get_position().x, get_position().y - sign(y_scroll_offset) * 0.1 * (global.fps * delta)))
	elif y_scroll_offset != 0:
		set_position(Vector2(get_position().x, get_position().y + (orig_y - get_position().y) / 8 * (global.fps * delta)))
		if abs(orig_y - get_position().y) <= 1:
			scrolling = true
	var white_amount = char_spr.get_material().get_shader_param("white_amount")
	if white_amount > 0:
		char_spr.get_material().set_shader_param("white_amount", white_amount - 0.05 * (global.fps * delta))
...
func select():
	if not selected:
		...
		if curr_char == global.CHAR.random:
			...
			char_spr.set_position(Vector2(char_spr.get_position().x, get_position().y + select_y))
		palette_num = - 1
		anim_player.play(anim_player.current_animation + "_select")
		char_spr.get_material().set_shader_param("white_amount", 1)
...
func back():
	if selected_palette:
		...
		palette_num = - 1
		set_palette()
		char_spr.set_position(Vector2(get_position().x + select_x, get_position().y + unselect_y))
...
func set_palette_color(palette_color, palette_num, default):
	if default:
		char_spr.get_material().set_shader_param("color_o" + str(palette_num), palette_color)
	else :
		char_spr.get_material().set_shader_param("color_n" + str(palette_num), palette_color)
