...
func _process(delta):
			...
			match packet_type:
				...
				global.P_TYPE.lobby_ready_start:
					if global.lobby_join:
						...
						if not check_ids.has(sender_id) and (sender_id == ready_ids[0] or sender_id == ready_ids[1]):
	...
	if menu_yesno.active:
		...
		if lobby_init and not started:
			if Input.is_action_just_pressed("player1_attack") and not lobby_chat_banner.is_active() and can_ready:
				if global.lobby_join:
					...
					if global.lobby_member_ids.size() > 2:
						...
						if global.steam_id == global.lobby_member_ids[1]:
							if global.lobby_member_ready[1] == global.LOBBY_READY.ready:
								...
								broadcast_packet_timer_player2_ready_start()
					check_start()
			if Input.is_action_just_pressed("player1_dash") and not lobby_chat_banner.is_active():
				var skip = bool(int(global.get_lobby_member_data(global.steam_id, global.MEMBER_LOBBY_SKIP)))
				global.set_lobby_member_data(global.MEMBER_LOBBY_SKIP, str(int( not skip)))
		if Input.is_action_just_pressed("player1_special") and not lobby_chat_banner.is_active():
	...
	label_timeout.visible = not timeout_timer.is_stopped()
	label_info.visible = not label_timeout.visible
	label_player1_ready.visible = not player1_ready_timer.is_stopped()
	label_player2_ready.visible = not player2_ready_timer.is_stopped()
...
func check_start():
	if global.lobby_member_ready[0] == global.LOBBY_READY.ready and global.lobby_member_ready[1] == global.LOBBY_READY.ready and not started:
		started = true
